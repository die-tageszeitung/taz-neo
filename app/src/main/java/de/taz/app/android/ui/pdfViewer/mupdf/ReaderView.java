...
public class ReaderView
    ...
    public boolean onSingleTapConfirmed(MotionEvent e) {
        if (!tapDisabled) {
            ...
            float docRelX = (e.getX() - pageView.getLeft()) / scale;
            float docRelY = (e.getY() - pageView.getTop()) / scale;

            onDocTap(docRelX, docRelY);
        }
        return true;
    }
    ...
    public boolean onDoubleTap(MotionEvent e) {
        ...
        float docRelX = e.getX() - view.getLeft() + mXScroll;
        float docRelY = e.getY() - view.getTop() + mYScroll;

        onDocDoubleTap(docRelX, docRelY);
     ...
     */
    public void zoomTo(float scale, float centerDocRelX, float centerDocRelY) {
        float scaleFactor = scale / mScale;
        mScale = scale;

        if (centerDocRelX != 0 || centerDocRelY != 0) {
            mXScroll += (centerDocRelX - centerDocRelX * scaleFactor);
            mYScroll += (centerDocRelY - centerDocRelY * scaleFactor);
        }
    ...
    protected void onDocTap(float docRelX, float docRelY) {

    }

    protected void onDocDoubleTap(float docRelX, float docRelY) {
    }
}
